...
class TLS_Context
   ...
   public:
      ...
      bool session_activated_called() const { return m_callbacks.session_activated_called; }
...
class Test_TLS_RFC8448 final : public Test
   ...
   private:
      Test::Result simple_1_rtt_client_hello()
         ...
         auto add_session_ticket_and_sort = [](Botan::TLS::Extensions& exts, Botan::TLS::Connection_Side side)
            {
         ...
         Client_Context ctx(std::move(rng), read_tls_policy("rfc8448_1rtt"), add_session_ticket_and_sort);
      ...
      Test::Result hello_retry_request()
         ...
         auto add_padding_and_sort = [flights = 0](Botan::TLS::Extensions& exts, Botan::TLS::Connection_Side side) mutable
            {
            sort_extensions(exts, side);
         ...
         Client_Context ctx(std::move(rng), read_tls_policy("rfc8448_hrr"), add_padding_and_sort);
         result.confirm("client not closed", !ctx.client.is_closed());
